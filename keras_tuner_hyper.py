...
def keras_build_model(hp, model_arch_str: str):
    ...
    if model_arch_str.lower() == "naivecnn":
        ...
        return_param_dict["pool_type"] = hp.Choice('pool', ['max', 'avg'], default='max')
        return_param_dict["loss_function"] = hp.Choice('loss', ['categorical_crossentropy', 'poisson', 'kullback_leibler_divergence'],
        ...
        return_param_dict['loss_function'] = hp.Choice('loss', ['categorical_crossentropy', 'poisson', 'kullback_leibler_divergence'], default='categorical_crossentropy')
        ...
        return_param_dict['pool_type'] = hp.Choice('pool', ['globalmax', 'globalavg'], default='globalmax')
        return_param_dict['loss_function'] = hp.Choice('loss', ['binary_crossentropy', 'poisson', 'kullback_leibler_divergence'], default='binary_crossentropy')
        return_param_dict['optimizer'] = hp.Choice('optimizer', ['SGD', 'RMSprop', 'Adam', 'Adadelta', 'Adagrad', 'Adamax', 'Nadam', 'Ftrl'], default='Adam')
        return_param_dict['learning_rate'] = hp.Float('learning_rate', 1e-4, 1e-1, sampling='log', default=1e-3)
        return_param_dict['batch_size'] = hp.Choice('batch_size', [16, 32, 64, 128, 256, 512], default=32)
